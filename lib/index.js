...
var Map = O.class('ose-level/lib/map');
...
exports.init = function(shard, params, cb) {  // {{{2
  shard.leveldb = params.schema || 'memdown';
...
exports.map = function(name, params) {  // {{{2
  ...
  var result = new Map(this, name, params);

  if (result.kind && ! result.map) {
    result.onePerEntry = true;

    if (result.field) {
      if (result.unique) {
        result.map = new Function('entry', 'cb',
          'if (entry.kind.name === "' + result.kind.name + '" && "' + result.field + '" in entry.dval) cb(entry.dval.' + result.field + ', entry.id);'
        );
        result.getId = new Function('key', 'val', 'return val');
      } else {
        result.map = new Function('entry', 'cb',
          'if (entry.kind.name === "' + result.kind.name + '" && "' + result.field + '" in entry.dval) cb([entry.dval.' + result.field + ', entry.id], null);'
        );
        result.getId = new Function('key', 'return key[1]');
      }
    } else {
      result.unique = true;
      result.map = new Function('entry', 'cb', 'if (entry.kind.name === "' + result.kind.name + '") cb(entry.id, null);');
      result.getId = new Function('key', 'return key');
    }
  }

  if (result.field && ! result.map) {
    result.onePerEntry = true;

    if (result.unique) {
      result.map = new Function('entry', 'cb', 'if ("' + result.field + '" in entry.dval) {cb(entry.dval.' + result.field + ', entry.id); }');
      result.getId = new Function('key', 'val', 'return val');
    } else {
      result.map = new Function('entry', 'cb', 'if ("' + result.field + '" in entry.dval) {cb([entry.dval.' + result.field + ', entry.id], null); }');
      result.getId = new Function('key', 'return key[1]');
    }
  }
};
