...
const O = require('ose')(module)
  .setPackage('ose-level')
  .class(init)
;

O.content('../content');
var Entry = O.getClass('ose/lib/entry');
var Map = O.getClass('./map');

/** Doc {{{1
 * @caption LevelDB
 *
 * @description
 * This package allows OSE to use LevelDB as a data store.
 *
 * @module level
 * @main level
 */
 ...
 *
 * @readme
 * This class can be used as a base class to create schemas using LevelDB as a data store.
 *
 * @description
  ...
  shard.awaitReady(function(err) {
    ...
    if (filter) {
      stream.on('data', function(data) {
        if (filter(data.key, data.value, opts.filter, shard)) resp.push(map.getId(data.key, data.value));
      });
    } else {
      stream.on('data', function(data) {
